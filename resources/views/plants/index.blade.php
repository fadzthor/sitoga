...
<head>
    ...
    <title>Daftar Tanaman Herbal</title>
...
<body
    ...
    <header class="w-full max-w-3xl mx-auto px-4 pt-10">
        <h1 class="text-3xl font-bold text-center mb-4">Koleksi Tanaman Obat</h1>
        <p class="text-center text-sm text-[#325749] dark:text-gray-300 mb-6">Temukan tanaman herbal pilihan untuk
            kesehatan alami Anda.</p>

        <form action="{{ route('plants.index') }}" method="GET" class="mb-8">
            <input type="text" name="search" placeholder="Cari tanaman..." value="{{ request('search') }}"
    ...
    <main class="w-full max-w-3xl mx-auto px-4 pb-20">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                @foreach ($plants as $plant)
                    <a href="{{ route('plants.show', $plant) }}"
                        ...
                        <img src="{{ $plant->photo ? asset('storage/' . $plant->photo) : 'https://placehold.co/200x200?text=No+Image' }}"
                            alt="{{ $plant->local_name }}"
                            class="w-28 h-28 object-cover rounded-full mb-3 border-4 border-[#cfe3d8] shadow">
                        <h3 class="text-lg font-semibold text-[#325749] dark:text-white">{{ $plant->local_name }}</h3>
                        @if ($plant->scientific_name)
                            <p class="text-sm italic text-gray-600 dark:text-gray-300">{{ $plant->scientific_name }}</p>
                        @endif
                    </a>
                @endforeach
            </div>
        ...
        @else
            <p class="text-center text-gray-700 dark:text-gray-300 text-lg font-medium">Tanaman tidak ditemukan.</p>
        @endif
    </main>
